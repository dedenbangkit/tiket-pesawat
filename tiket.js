...
prompt.get([{
  properties: {
    dari: {
      description: color.cyan('Kota Keberangkatan:')
    }
  }
},
{
  properties: {
    ke: {
      description: color.cyan('Kota Tujuan:')
    }
  }
},
{
  properties: {
    tanggal: {
      description: color.cyan('Tanggal (YY-MM-DD):')
    }
  }
},
{
  properties: {
    dewasa: {
      description: color.cyan('Penumpang Dewasa:')
    }
  }
},
{
  properties: {
    anak: {
      description: color.cyan('Anak-anak (diatas 2 tahun):')
    }
  }
},
{
  properties: {
    infant: {
      description: color.cyan('Bayi (dibawah 2 tahun):')
    }
  }
}
], function (err, result) {
  ...
  request('https://www.tiket.com/pesawat/cari?d=' + from + '&a=' + to + '&date=' + date + '&adult=' + people + '&child=' + child + '&infant=' + infant, function (error, response, html) {
    ...
    if (price.length === 0) {
      console.log(color.red('no schedule found'))
    }
  })
})
